...
The Deye/Solarman inverter integration (`backend/.../adapter/out/inverter/solarman/`) has been verified end-to-end against real hardware: the logger's IP/port/serial number and the battery SOC register work correctly, cross-checked against the Solarman app's reported percentage. The grid-charge enable/target registers are still placeholders — they need to be identified for this inverter model before the app can control charging.
