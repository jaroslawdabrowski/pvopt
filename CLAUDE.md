...
**The TOU slot *times* need a one-off rewrite before any of this is trustworthy.** The device ships with slots at 01:00/05:00/09:00/13:00/17:00/21:00, which don't line up with G12's real boundaries (e.g. the "21:00" slot also covers 21:00–22:00, which is expensive). `TouSlotSetupManualTest` (same manual-diagnostic pattern as `RegisterScannerManualTest` below, `@Disabled` by default) rewrites registers 148–153 to `TouSlots.START_TIMES_HHMM` (00:00/06:00/13:00/15:00/21:00/22:00 — 21:00 is a deliberate filler slot, always OFF, G12 has no real boundary there) and nothing else. It requires **both** `-Dpvopt.setup.confirm=yes` and `-Dpvopt.setup.writeEnabled=true` to actually write (prints old→new and dry-runs otherwise) — run it once, by hand, before ever setting `pvopt.inverter.solarman.write-enabled=true` for normal operation.

**Safety switch, born from a real incident**: an earlier, since-removed hourly scheduler once fired against unverified placeholder registers (values happened to read back unchanged afterward — luck, not by design). `SolarmanInverterConfig.writeEnabled` (`pvopt.inverter.solarman.write-enabled`, **default `false`**) now gates every actual Modbus write in `SolarmanInverterAdapter.applyChargeSchedule` (and in `TouSlotSetupManualTest`) — with it false, the adapter only logs what it would have written. `SolarmanInverterAdapterTest.doesNotTouchTheNetworkWhenWriteIsDisabled` is a regression guard (points the adapter at an unroutable host — RFC 5737 TEST-NET-1 — and asserts nothing throws, proving no socket is ever opened when disabled). **Don't flip `write-enabled` to `true` until `TouSlotSetupManualTest` has been run and verified on the LCD** — the two daily scheduled jobs run unattended, forever, once it's on.
