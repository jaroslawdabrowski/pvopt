...
The register addresses in `SolarmanInverterConfig` (`battery-soc-register`, `grid-charge-enable-register`, `grid-charge-target-soc-register`) and the connection details (`pvopt.inverter.solarman.host/port/logger-serial`) are placeholders pending hardware verification against the user's actual inverter/logger — don't treat the current values as correct without checking against a packet capture or the actual Deye model's register map (community references: `pysolarmanv5`, Home Assistant's `solarman` integration, `deye-inverter-mqtt`).
