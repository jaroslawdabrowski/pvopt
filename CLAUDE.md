...
The real inverter's Time-of-Use table (6 slots, each with Time/Power/Batt%/Gen/Grid Charge — visible on the inverter's own LCD under System Work Mode) is confirmed to live at holding registers 148–177, one register per slot per field, *not* single registers: Time `148–153` (HHMM, e.g. `100`=01:00, `1700`=17:00), Power `154–159`, Batt% `166–171`. `160–165` (a block of 6 identical values) is an unidentified column not shown on that screen. One clean 6-register block of zeros at `172–177` is very likely one of Gen/Grid-Charge-enable (both were off on the real device, so it can't be distinguished from the other by value alone yet) — the second block hasn't been located. This means the *actual* device has one grid-charge-enable and one grid-charge-target-SOC value **per time slot**, not the single pair `SolarmanInverterConfig.gridChargeEnableRegister`/`gridChargeTargetSocRegister` currently models — that's a real design gap to resolve (e.g. always write to whichever slot covers "now") before wiring up writes for real.
