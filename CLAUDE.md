...
The real inverter's Time-of-Use table (6 slots, each with Time/Power/Batt%/Gen/Grid Charge — visible on the inverter's own LCD under System Work Mode) is confirmed to live at holding registers 148–177, one register per slot per field, *not* single registers: Time `148–153` (HHMM, e.g. `100`=01:00, `1700`=17:00), Power `154–159`, Batt% `166–171`, **Grid Charge enable `172–177`** (confirmed live: toggling slot 1's checkbox on the LCD flipped register 172 from `0` to `1`, nothing else changed). `160–165` (a block of 6 identical values) is an unidentified column not shown on that screen; the Gen-enable block hasn't been located (not needed for grid charging). This means the *actual* device has one grid-charge-enable and one target-SOC value **per time slot**, not the single pair `SolarmanInverterConfig.gridChargeEnableRegister`/`gridChargeTargetSocRegister` currently models — that's a real design gap to resolve (e.g. always write to whichever slot covers "now", or write all 6 slots) before wiring up writes for real. The target-SOC-per-slot values are the same Batt% registers already confirmed at `166–171`.

**Safety incident, now guarded against**: the hourly `@Scheduled` job calls `applyChargeSchedule()` regardless of whether its target registers have been verified — it isn't aware "these are placeholders." It already fired once against the placeholder `145`/`146` (values happened to read back unchanged afterward, so no apparent harm, but that was luck, not by design). `SolarmanInverterConfig.writeEnabled` (`pvopt.inverter.solarman.write-enabled`, **default `false`**) now gates every actual Modbus write in `SolarmanInverterAdapter.applyChargeSchedule` — with it false, the adapter only logs what it would have written. `SolarmanInverterAdapterTest.doesNotTouchTheNetworkWhenWriteIsDisabled` is a regression guard (points the adapter at an unroutable host — RFC 5737 TEST-NET-1 — and asserts nothing throws, proving no socket is ever opened when disabled). **Do not flip `write-enabled` to `true` until `gridChargeEnableRegister`/`gridChargeTargetSocRegister` are confirmed against real hardware** (and the per-time-slot design gap above is resolved) — the scheduler runs unattended, hourly, forever.
