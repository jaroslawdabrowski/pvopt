...
The connection details (`pvopt.inverter.solarman.host/port/logger-serial`) are **confirmed working** against the real logger on the LAN — the Solarman V5 handshake and a Modbus RTU register read both succeed end-to-end. The register addresses in `SolarmanInverterConfig` (`battery-soc-register`, `grid-charge-enable-register`, `grid-charge-target-soc-register`) are still placeholders, though — `battery-soc-register=184` reads back `1`, which isn't a plausible SOC value, so the real register map for this inverter model still needs to be identified (community references: `pysolarmanv5`, Home Assistant's `solarman` integration, `deye-inverter-mqtt`) before relying on any of the three registers, especially the two grid-charge ones used for writes.
